...
def homework_reduction_source(partitions):
    ...
    #Create a variable that refers to the value of the size of the  array divided by the number of
    #   partitions; this was done for convenience
    function_body = "   int sop = size/"+str(partitions)+";\n"
    
    #Create a loop that iterates over each element not of the array, but of each partition in the
    #   array
    function_body +="   for (int x=1; x<sop; ++x) {\n"

    #For each partition in the array, access the first element of the partition, then add each non-first
    #   element in the partition to that first element
    for x in range(partitions):
        function_body +="       a[sop*"+str(x)+"] += a[(sop*"+str(x)+")+x];\n"
    function_body +="   }\n\n"
    
    #Add the first element of each non-first partition to the first element in the array
    function_body +="   for (int x=1; x<"+str(partitions)+"; ++x)\n"
    function_body +="       a[0] += a[sop*x];\n"
    function_close = "}"
